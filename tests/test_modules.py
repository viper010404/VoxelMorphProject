...
def test_spatial_transformer_batched_dispatch_2d():
    """
    Regression test: SpatialTransformer must use batched grid_sample, not per-element loop.

    A vectorized implementation calls grid_sample once per forward pass.
    A looped implementation calls it batch_size times, causing:
    - Kernel launch overhead
    - Suboptimal memory access patterns
    - Slower execution

    This test ensures we don't regress to per-element dispatch.
    """
    from unittest.mock import patch

    batch_size = 4
    transformer = vxm_modules.SpatialTransformer()
    moving = torch.randn(batch_size, 1, 32, 32)
    disp = torch.randn(batch_size, 2, 32, 32)

    call_count = 0
    original_grid_sample = torch.nn.functional.grid_sample

    def counting_grid_sample(*args, **kwargs):
        "wrap grid sample with a counter"
        nonlocal call_count
        call_count += 1
        return original_grid_sample(*args, **kwargs)

    with patch.object(torch.nn.functional, 'grid_sample', counting_grid_sample):
        _ = transformer(moving, disp)

    assert call_count == 1, (
        f"SpatialTransformer should call grid_sample once (batched), but called it {call_count}"
        "times resulting in kernel launch overhead and suboptimal memory patterns. Must be batched"
    )
